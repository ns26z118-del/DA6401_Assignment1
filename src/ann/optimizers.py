...
class NAG:
    ...
    Standard update (Bengio et al. reformulation):
        v_t = beta * v_{t-1} + lr * grad_w
        w  -= beta * v_t + lr * grad_w
    """
    def __init__(self, lr, weight_decay=0.0, beta=0.9):
        self.lr = lr
        self.wd = weight_decay
    ...
    def step(self, layers):
        for i, layer in enumerate(layers):
            if hasattr(layer, "W"):
                ...
                # Nesterov update
                layer.W -= self.beta * self.v_W[i] + self.lr * grad_W
                layer.b -= self.beta * self.v_b[i] + self.lr * grad_b
